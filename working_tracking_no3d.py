#!/usr/bin/env python3
"""
Working ZED2 Tracking System - Without 3D Point Cloud Lookups
Based on diagnostic findings: 3D lookups cause freezing, so we'll use depth map instead
"""
...
class WorkingTrackingSystem:
    def __init__(self, config_path: str = "config.json"):
        """Initialize working tracking system"""
        print("Initializing working tracking system (no 3D point cloud lookups)...")
        ...
        self.depth_map = sl.Mat()  # Use depth map instead of point cloud
        ...
        print("Working tracking system ready!")
    ...
    def get_depth_at_point(self, x: int, y: int) -> Optional[float]:
        """Get depth at specific pixel using depth map (safer than point cloud)"""
        try:
            # Get depth value directly from depth map
            depth_value = self.depth_map.get_value(x, y)
            
            # Check if depth is valid (finite and positive)
            if np.isfinite(depth_value) and depth_value > 0.1:
                return float(depth_value)
        except Exception as e:
            # Silently handle any depth lookup errors
            pass
        
        return None

    def detect_objects(self, image: np.ndarray) -> List[Detection]:
        """Run YOLO detection with safe depth lookup"""
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Get depth using safer depth map lookup
    ...
    def select_tracking_target(self, detections: List[Detection]) -> Optional[Detection]:
        ...
        if not valid_detections:
            # If no depth data available, use all detections and prioritize by confidence
            if detections:
    ...
    def run_tracking(self, duration: Optional[float] = None):
        ...
        print("Controls: 'q' to quit, 'r' to reset tracking")
        
        # Setup display
        cv2.namedWindow("ZED2 Object Tracking", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("ZED2 Object Tracking", 1280, 720)
        ...
        try:
            while True:
                ...
                # Get images - only what we need
                self.zed.retrieve_image(self.left_image, sl.VIEW.LEFT)
                self.zed.retrieve_measure(self.depth_map, sl.MEASURE.DEPTH)  # Use depth map, not point cloud
                ...
                if detections:
                    ...
                    if self.tracking_target:
                        ...
                        # Here you would send angles to servo controller
                        # For now, just track the calculation
                ...
                if self.tracking_target:
                    ...
                    status_lines.append(f"Angles: Pan {pan_angle:.1f}°, Tilt {tilt_angle:.1f}°")
                ...
                cv2.imshow("ZED2 Object Tracking", vis)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Working ZED2 Tracking System")
    ...
    try:
        tracker = WorkingTrackingSystem(args.config)
